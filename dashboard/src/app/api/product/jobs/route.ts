...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Select a configured workspace and provide a specific delivery request." }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "The execution plane could not accept the job." }, { status: 502 });
  }
}
